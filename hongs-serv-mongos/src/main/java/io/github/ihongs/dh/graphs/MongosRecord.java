 ...
 * @author Hongs
 */
public class MongosRecord extends ModelCase implements IEntity, ITrnsct, AutoCloseable {
    ...
    private Coll db = null;

    public MongosRecord(Map form) {
     ...
     */
    public static MongosRecord getInstance(String conf, String form)
    throws HongsException {
        String code = MongosRecord.class.getName( ) +":"+ conf +"."+ form;
        Core   core = Core.getInstance( );
        if ( ! core.containsKey( code ) ) {
            Map  fxrm = FormSet.getInstance(conf).getForm(form);
            MongosRecord inst = new MongosRecord(fxrm);
            core.put( code, inst ) ; return inst ;
        } else {
            return  (MongosRecord) core.got(code);
        }
    }
    ...
    public static class Coll implements AutoCloseable {
         ...
         * @return
         */
        public static Coll getInstance(String conf, String form) {
            try {
                Map opts = (Map) FormSet
                      .getInstance(conf)
                      .getForm    (form)
